...
// Full procedural fantasy world generator with SVG rendering
// ═══════════════════════════════════════════════════════════════════════════
...
const NAME_PREFIXES = {
  mountain: ["Iron","Storm","Thunder","Grey","Black","White","Frost","Stone","Dragon","Eagle","Cloud","Silver","Shadow","Red","Dark","Ancient","Bone","Fire","Wind","Grim"],
  forest: ["Green","Dark","Whisper","Ancient","Shadow","Thorn","Moss","Silver","Twilight","Elder","Deep","Wild","Fey","Moon","Mystic","Pale","Amber","Golden","Hollow","Twisted"],
  city: ["King's","Queen's","Gold","Silver","Iron","Bright","High","Storm","River","Lake","Star","Crown","Shield","Dragon","Lion","Eagle","Falcon","Wolf","Crimson","Azure"],
  town: ["Mill","Oak","Elm","Ash","Fox","Deer","Copper","Stone","Brook","Green","Thorn","Rose","Ivy","Fern","Birch","Willow","Reed","Moss","Hazel","Clover"],
  ruins: ["Lost","Fallen","Broken","Forsaken","Cursed","Shattered","Silent","Forgotten","Dead","Hollow","Blighted","Sunken","Buried","Haunted","Void","Doom","Ash","Ghost","Shadow","Wither"],
  dungeon: ["Shadow","Deep","Dark","Bone","Skull","Doom","Dread","Iron","Black","Blood","Void","Worm","Spider","Rot","Crypt","Tomb","Pit","Abyss","Nether","Plague"],
  lake: ["Crystal","Mirror","Silver","Moon","Still","Deep","Blue","Clear","Silent","Glass","Frost","Starlight","Azure","Sapphire","Mist","Dream","Shimmer","Pale","Dawn","Twilight"],
  river: ["Silver","Serpent","Swift","Rush","Winding","Long","Dark","Cold","White","Wild","Rapid","Gentle","Broad","Deep","Clear","Bright","Spring","Storm","Thunder","Iron"],
  swamp: ["Murk","Black","Dead","Rot","Mire","Foul","Bog","Slime","Stink","Gloom","Blight","Dread","Vile","Dark","Poison","Plague","Wither","Stagnant","Fetid","Rank"],
  desert: ["Sun","Scorched","Burning","Red","Golden","Dry","Dust","Sand","Bone","Fire","Blaze","Ember","Heat","Wither","Ash","Shimmering","Barren","Desolate","Parched","Blazing"],
  island: ["Storm","Sea","Coral","Pearl","Shell","Tide","Wave","Wind","Salt","Spray","Mist","Fog","Drift","Anchor","Harbor","Haven","Lost","Forgotten","Hidden","Phantom"],
  kingdom: ["Grand","High","Noble","Royal","Ancient","Mighty","Golden","Silver","Iron","Crystal","Sacred","Eternal","Divine","Blessed","Sovereign","Imperial","Hallowed","Radiant","Glorious","Celestial"],
};

const NAME_SUFFIXES = {
  mountain: ["peak","crag","horn","spire","fang","ridge","tor","summit","mount","cliff","crest","needle","crown","tooth","pillar","stone","fall","reach","top","throne"],
  forest: ["wood","grove","weald","glen","thicket","copse","shade","hollow","dell","brake","glade","holt","bough","leaf","vale","wild","deep","heart","tangle","root"],
  city: ["haven","gate","hold","keep","port","ford","burg","vale","ward","cross","bridge","tower","hall","march","stead","borough","court","reach","field","throne"],
  town: ["shire","bury","dale","ton","wick","ham","ford","well","field","vale","stead","bridge","hollow","croft","thorpe","leigh","worth","mere","mead","beck"],
  ruins: ["spire","gate","keep","hold","hall","throne","temple","altar","sanctum","vault","bastion","tower","citadel","palace","crypt","tomb","barrow","cairn","mound","obelisk"],
  dungeon: ["depths","pit","lair","den","cave","hollow","warren","maze","labyrinth","catacomb","dungeon","vault","cell","chamber","grotto","abyss","chasm","burrow","tunnels","maw"],
...
const NAME_STANDALONE = {
  mountain: ["Dragonspine","Worldpeak","Skyreach","Cloudpiercer","Thundertop","Stormcrown","Ironspire","Frostfang","Shadowcrest","Eaglenest","Bonecrag","Firehorn","Windshear","Grimtooth","Stonethrone"],
  forest: ["Thornwood","Darkhollow","Greenmantle","Shadowbark","Moongrove","Elderweald","Whisperleaf","Silverbough","Deeproot","Wiltshire","Fernglade","Nightshade","Mistwood","Hexwood","Briarheart"],
  city: ["Astoria","Meridian","Valdris","Ironhaven","Stormgate","Brighthollow","Kingsreach","Solara","Nordheim","Caldera","Arcanis","Luminara","Thronehall","Dragoncrest","Starfall"],
  sea: ["The Sapphire Deep","The Endless Blue","Mare Tenebris","The Churning Expanse","The Abyssal Main","The Crystal Tide","The Storm Sea","The Dragon's Wake","The Frozen Deep","The Merchant's Way"],
};

function generateName(rng, type) {
  const r = rng();
  // 30% chance of standalone name
  if (r < 0.3 && NAME_STANDALONE[type]) {
    const arr = NAME_STANDALONE[type];
    return arr[Math.floor(rng() * arr.length)];
  }
  const prefixes = NAME_PREFIXES[type] || NAME_PREFIXES.city;
  const suffixes = NAME_SUFFIXES[type] || NAME_SUFFIXES.city;
  const pre = prefixes[Math.floor(rng() * prefixes.length)];
  const suf = suffixes[Math.floor(rng() * suffixes.length)];
  // 50% chance of combined word, 50% of two words
  if (rng() < 0.5) {
    return pre + suf;
  }
...
function generateKingdomName(rng) {
  const r = rng();
  const prefixes = NAME_PREFIXES.kingdom;
  const suffixes = NAME_SUFFIXES.kingdom;
  const pre = prefixes[Math.floor(rng() * prefixes.length)];
  const suf = suffixes[Math.floor(rng() * suffixes.length)];
  if (r < 0.25) return "The " + pre + " " + suf.charAt(0).toUpperCase() + suf.slice(1);
  if (r < 0.5) return pre + " " + suf.charAt(0).toUpperCase() + suf.slice(1) + " of " + generateName(rng, "city");
  if (r < 0.75) return "Kingdom of " + generateName(rng, "city");
  return "The " + suf.charAt(0).toUpperCase() + suf.slice(1) + " of " + pre.toLowerCase() + " " + generateName(rng, "mountain").split(" ")[0].toLowerCase();
}

// ─── TERRAIN GENERATION (Simplex-like noise via seeded RNG) ────────────────

function generateHeightmap(width, height, rng, octaves = 6, persistence = 0.5, lacunarity = 2.0) {
  // Multi-octave value noise with interpolation
  const grid = [];
  ...
  // Create base noise grids for each octave
  for (let oct = 0; oct < octaves; oct++) {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        const val = (n00 * (1 - sx) + n10 * sx) * (1 - sy) + (n01 * (1 - sx) + n11 * sx) * sy;
        grid[y][x] += val * amp;
      }
    }
  }
...
function generateMoisture(width, height, rng) {
  return generateHeightmap(width, height, rng, 4, 0.6, 2.2);
}

function generateTemperature(width, height, rng, latBias = true) {
  const base = generateHeightmap(width, height, rng, 3, 0.5, 2.0);
  if (latBias) {
    for (let y = 0; y < height; y++) {
      const lat = Math.abs(y / height - 0.5) * 2; // 0 at equator, 1 at poles
      for (let x = 0; x < width; x++) {
        base[y][x] = base[y][x] * 0.4 + (1 - lat) * 0.6;
      }
    }
  }
  return base;
...
const BIOMES = {
  deepOcean:    { color:"#1a3a5c", label:"Deep Ocean",      minH:-1, maxH:0.22, minM:0, maxM:1, minT:0, maxT:1 },
  ocean:        { color:"#2a5a8c", label:"Ocean",            minH:0.22, maxH:0.32, minM:0, maxM:1, minT:0, maxT:1 },
  shallows:     { color:"#3a7ab0", label:"Shallow Water",    minH:0.32, maxH:0.38, minM:0, maxM:1, minT:0, maxT:1 },
  beach:        { color:"#d4c088", label:"Beach",            minH:0.38, maxH:0.42, minM:0, maxM:1, minT:0.3, maxT:1 },
  tundra:       { color:"#c8d8e0", label:"Tundra",           minH:0.38, maxH:0.65, minM:0, maxM:0.3, minT:0, maxT:0.25 },
  snowfield:    { color:"#eef4f8", label:"Snowfield",        minH:0.38, maxH:0.65, minM:0.3, maxM:1, minT:0, maxT:0.2 },
  grassland:    { color:"#7da668", label:"Grassland",        minH:0.42, maxH:0.58, minM:0.2, maxM:0.5, minT:0.35, maxT:0.75 },
  plains:       { color:"#a8c070", label:"Plains",           minH:0.42, maxH:0.55, minM:0.15, maxM:0.45, minT:0.4, maxT:0.85 },
  savanna:      { color:"#c8b050", label:"Savanna",          minH:0.42, maxH:0.58, minM:0.1, maxM:0.35, minT:0.6, maxT:1 },
  desert:       { color:"#d4a860", label:"Desert",           minH:0.42, maxH:0.65, minM:0, maxM:0.15, minT:0.5, maxT:1 },
  forest:       { color:"#4a7a3a", label:"Temperate Forest", minH:0.45, maxH:0.65, minM:0.5, maxM:0.8, minT:0.35, maxT:0.7 },
  denseForest:  { color:"#2d5a20", label:"Dense Forest",     minH:0.45, maxH:0.65, minM:0.7, maxM:1, minT:0.35, maxT:0.7 },
  jungle:       { color:"#1a6030", label:"Jungle",           minH:0.42, maxH:0.6, minM:0.7, maxM:1, minT:0.7, maxT:1 },
  swamp:        { color:"#4a6a3a", label:"Swamp",            minH:0.38, maxH:0.45, minM:0.6, maxM:1, minT:0.4, maxT:0.8 },
  hills:        { color:"#8a9a60", label:"Hills",            minH:0.58, maxH:0.68, minM:0.15, maxM:0.7, minT:0.25, maxT:0.8 },
  highlands:    { color:"#9a8a68", label:"Highlands",        minH:0.65, maxH:0.75, minM:0.1, maxM:0.5, minT:0.2, maxT:0.6 },
  mountain:     { color:"#8a8078", label:"Mountain",         minH:0.72, maxH:0.88, minM:0, maxM:1, minT:0, maxT:1 },
  snowMountain: { color:"#d8dce0", label:"Snow Peak",        minH:0.85, maxH:1.1, minM:0, maxM:1, minT:0, maxT:1 },
  volcanic:     { color:"#5a3030", label:"Volcanic",         minH:0.78, maxH:1.1, minM:0, maxM:0.2, minT:0.6, maxT:1 },
};

function classifyBiome(h, m, t) {
  // Water biomes by height alone
  if (h < 0.22) return "deepOcean";
  if (h < 0.32) return "ocean";
  if (h < 0.38) return "shallows";

  // High terrain
  if (h > 0.85) return "snowMountain";
  if (h > 0.78 && m < 0.2 && t > 0.6) return "volcanic";
  ...
  // Beach
  if (h < 0.42) {
    if (m > 0.6 && t > 0.4) return "swamp";
    if (t < 0.25) return "tundra";
    return "beach";
  }

  // Cold biomes
  if (t < 0.2) return m > 0.3 ? "snowfield" : "tundra";
  if (t < 0.25) return "tundra";

  // Hot + dry
  if (t > 0.5 && m < 0.15) return "desert";
  if (t > 0.6 && m < 0.35) return "savanna";

  // Wet + hot
  if (t > 0.7 && m > 0.7) return "jungle";

  // Wet temperate
  if (m > 0.7) return "denseForest";
...
function generateRivers(heightmap, moistureMap, gridW, gridH, rng, count = 8) {
  const rivers = [];
  const seaLevel = 0.38;

  for (let i = 0; i < count; i++) {
    // Find a highland source
    let sx, sy, attempts = 0;
    do {
      sx = Math.floor(rng() * gridW);
      sy = Math.floor(rng() * gridH);
      attempts++;
    } while (attempts < 200 && (heightmap[sy]?.[sx] || 0) < 0.6);
    ...
    for (let step = 0; step < 300; step++) {
      const h = heightmap[cy]?.[cx] || 0;
      if (h < seaLevel) break; // reached ocean
...
function placeCities(heightmap, moistureMap, rivers, gridW, gridH, rng, config) {
  const seaLevel = 0.38;
  const settlements = [];
  const minDist = { capital: 40, city: 25, town: 15, village: 8, hamlet: 5 };
  const counts = {
    capital: config.capitals || 3,
    city: config.cities || 8,
    town: config.towns || 15,
    village: config.villages || 25,
    hamlet: config.hamlets || 35,
  };

  // River proximity map
  const riverProx = Array.from({ length: gridH }, () => new Float32Array(gridW));
      ...
      for (let dy = -5; dy <= 5; dy++) {
        for (let dx = -5; dx <= 5; dx++) {
          ...
          if (nx >= 0 && nx < gridW && ny >= 0 && ny < gridH) {
            ...
            riverProx[ny][nx] = Math.max(riverProx[ny][nx], 1 - dist / 6);
          }
        }
      }
    }
  ...
  // Coast proximity map
  const coastProx = Array.from({ length: gridH }, () => new Float32Array(gridW));
      ...
      if (heightmap[y][x] >= seaLevel) {
        for (let dy = -4; dy <= 4; dy++) {
          for (let dx = -4; dx <= 4; dx++) {
            ...
            if (nx >= 0 && nx < gridW && ny >= 0 && ny < gridH && heightmap[ny][nx] < seaLevel) {
              coastProx[y][x] = 1;
              break;
            }
          }
          if (coastProx[y][x] > 0) break;
        }
      }
    }
  }

  function distToSettlements(x, y) {
    let minD = Infinity;
  ...
  function scoreCell(x, y, type) {
    ...
    if (h < seaLevel || h > 0.78) return -1; // water or too high
    const m = moistureMap[y]?.[x] || 0;
    let score = 0;
    score += riverProx[y][x] * 30; // near rivers
    score += coastProx[y][x] * 20; // near coast
    score += m * 10; // fertile land
    score -= Math.abs(h - 0.5) * 15; // prefer mid-elevation
    if (type === "capital") score += (riverProx[y][x] > 0.3 ? 20 : 0) + (coastProx[y][x] > 0 ? 15 : 0);
    return score;
  }

  const typeOrder = ["capital", "city", "town", "village", "hamlet"];
  for (const type of typeOrder) {
    for (let i = 0; i < counts[type]; i++) {
      ...
      for (let attempt = 0; attempt < 500; attempt++) {
        ...
        const d = distToSettlements(x, y);
        if (d < minDist[type]) continue;
      ...
      if (bestX >= 0) {
        ...
        settlements.push({
          x: bestX, y: bestY, type,
          name: generateName(rng, nameType),
          population: type === "capital" ? 10000 + Math.floor(rng() * 40000) :
                      type === "city" ? 3000 + Math.floor(rng() * 15000) :
                      type === "town" ? 500 + Math.floor(rng() * 3000) :
                      type === "village" ? 50 + Math.floor(rng() * 500) :
                      10 + Math.floor(rng() * 50),
          isCoastal: coastProx[bestY]?.[bestX] > 0,
          isRiverside: riverProx[bestY]?.[bestX] > 0.3,
        });
      }
    }
  }
...
function generateRoads(settlements, heightmap, gridW, gridH, rng) {
  const roads = [];
  const seaLevel = 0.38;

  // Connect each city to its nearest neighbors using A* simplified
  const majorSettlements = settlements.filter(s => s.type === "capital" || s.type === "city");
  const allConnectable = settlements.filter(s => s.type !== "hamlet");

  // Simple road: bresenham with terrain avoidance
  function findPath(s1, s2) {
    ...
    for (let i = 0; i <= steps; i++) {
      const t = i / steps;
      let x = Math.round(s1.x + dx * t);
      let y = Math.round(s1.y + dy * t);
      // Slight terrain avoidance
      const h = heightmap[y]?.[x] || 0;
      if (h < seaLevel || h > 0.75) {
        // Try to wiggle around
        for (const off of [-1, 1, -2, 2]) {
          const ny = y + off;
          const nh = heightmap[ny]?.[x] || 0;
          if (nh >= seaLevel && nh < 0.75) { y = ny; break; }
        }
      }
      points.push({ x, y });
    }
  ...
  // Major roads: capitals to each other and nearest cities
  for (let i = 0; i < majorSettlements.length; i++) {
    // Find 2-3 nearest major settlements
    const dists = majorSettlements
      .map((s, j) => ({ s, j, d: Math.sqrt((s.x - majorSettlements[i].x) ** 2 + (s.y - majorSettlements[i].y) ** 2) }))
      .filter(x => x.j !== i)
      .sort((a, b) => a.d - b.d)
      .slice(0, 3);

    for (const { s: target } of dists) {
      const path = findPath(majorSettlements[i], target);
      if (path.length > 0) {
        roads.push({ path, type: "major", from: majorSettlements[i].name, to: target.name });
      }
    }
  }

  // Minor roads: towns to nearest city
  const towns = settlements.filter(s => s.type === "town");
  for (const town of towns) {
    const nearest = majorSettlements
      .map(s => ({ s, d: Math.sqrt((s.x - town.x) ** 2 + (s.y - town.y) ** 2) }))
      .sort((a, b) => a.d - b.d)[0];
    if (nearest && nearest.d < 60) {
      roads.push({ path: findPath(town, nearest.s), type: "minor", from: town.name, to: nearest.s.name });
    }
  }
...
function generateKingdoms(settlements, heightmap, gridW, gridH, rng, count = 5) {
  const capitals = settlements.filter(s => s.type === "capital");
  const kingdoms = [];
  const KINGDOM_COLORS = [
    "rgba(148,57,57,0.15)", "rgba(57,100,148,0.15)", "rgba(57,148,80,0.15)",
    "rgba(148,120,57,0.15)", "rgba(120,57,148,0.15)", "rgba(57,148,140,0.15)",
    "rgba(148,57,120,0.15)", "rgba(80,120,57,0.15)",
  ];
  const KINGDOM_BORDERS = [
    "rgba(148,57,57,0.5)", "rgba(57,100,148,0.5)", "rgba(57,148,80,0.5)",
    "rgba(148,120,57,0.5)", "rgba(120,57,148,0.5)", "rgba(57,148,140,0.5)",
    "rgba(148,57,120,0.5)", "rgba(80,120,57,0.5)",
  ];
  ...
  for (let i = 0; i < Math.min(count, capitals.length); i++) {
    ...
    for (const s of settlements) {
      if (s === cap) continue;
      const d = Math.sqrt((s.x - cap.x) ** 2 + (s.y - cap.y) ** 2);
      // Check if this is the nearest capital
      let isNearest = true;
      for (let j = 0; j < Math.min(count, capitals.length); j++) {
        if (j === i) continue;
        const d2 = Math.sqrt((s.x - capitals[j].x) ** 2 + (s.y - capitals[j].y) ** 2);
        if (d2 < d) { isNearest = false; break; }
      }
      if (isNearest && d < 80) mySettlements.push(s);
    }

    // Compute convex hull of settlement positions for territory border
    const points = mySettlements.map(s => ({ x: s.x, y: s.y }));
    if (points.length < 3) {
      // Expand with a radius
      const r = 15;
      for (let a = 0; a < Math.PI * 2; a += Math.PI / 4) {
        points.push({ x: cap.x + Math.cos(a) * r, y: cap.y + Math.sin(a) * r });
      }
    }
    ...
    const expanded = sorted.map(p => {
      ...
      return { x: p.x + (dx / len) * 12, y: p.y + (dy / len) * 12 };
    });

    kingdoms.push({
      name: generateKingdomName(rng),
      capital: cap.name,
      settlements: mySettlements.map(s => s.name),
      hull: expanded,
      color: KINGDOM_COLORS[i % KINGDOM_COLORS.length],
      borderColor: KINGDOM_BORDERS[i % KINGDOM_BORDERS.length],
      population: mySettlements.reduce((s, st) => s + (st.population || 0), 0),
    });
  }
  return kingdoms;
}
...
function generatePOIs(heightmap, moistureMap, settlements, gridW, gridH, rng, config) {
  const pois = [];
  const seaLevel = 0.38;
  const types = [
    { type:"dungeon", count: config.dungeons || 12, terrain:"high", icon:"dungeon" },
    { type:"ruins", count: config.ruins || 10, terrain:"any", icon:"ruins" },
    { type:"tower", count: config.towers || 8, terrain:"high", icon:"castle" },
    { type:"shrine", count: config.shrines || 10, terrain:"any", icon:"ruins" },
    { type:"cave", count: config.caves || 10, terrain:"mountain", icon:"dungeon" },
    { type:"grove", count: config.groves || 6, terrain:"forest", icon:"forest" },
    { type:"monolith", count: config.monoliths || 5, terrain:"any", icon:"ruins" },
    { type:"mine", count: config.mines || 8, terrain:"mountain", icon:"dungeon" },
    { type:"fortress", count: config.fortresses || 6, terrain:"high", icon:"castle" },
    { type:"temple", count: config.temples || 5, terrain:"any", icon:"ruins" },
    { type:"lighthouse", count: config.lighthouses || 4, terrain:"coast", icon:"castle" },
    { type:"camp", count: config.camps || 8, terrain:"any", icon:"town" },
  ];
  ...
  for (const { type, count, terrain, icon } of types) {
    for (let i = 0; i < count; i++) {
      ...
      if (bestX >= 0) {
        const nameType = type === "mine" || type === "cave" ? "dungeon" : type === "fortress" || type === "tower" || type === "lighthouse" ? "ruins" : type;
        pois.push({
          x: bestX, y: bestY, type, icon,
          name: generateName(rng, nameType),
          description: generatePOIDescription(rng, type),
          threat: ["low","medium","high","extreme"][Math.floor(rng() * 4)],
          discovered: rng() > 0.5,
        });
      }
    }
...
function generatePOIDescription(rng, type) {
  const descs = {
    dungeon: ["A sprawling underground complex","An ancient labyrinth of stone corridors","A dank network of tunnels","A forbidding subterranean stronghold","A crumbling underground fortress"],
    ruins: ["Crumbling remnants of a lost civilization","Overgrown stone foundations","A weathered monument to forgotten glory","Ancient columns reaching toward the sky","Half-buried walls etched with old runes"],
    tower: ["A lone watchtower on the heights","A crumbling wizard's spire","A fortified signal tower","An abandoned observatory","A dark sorcerer's retreat"],
    shrine: ["A weathered stone altar","A sacred grove with glowing runes","An ancient prayer site","A small temple to forgotten gods","A moss-covered devotional site"],
    cave: ["A deep natural cavern","A winding cave system","A dark grotto","A cavernous opening in the rock","A crystal-lined cave"],
    grove: ["An enchanted clearing","A circle of ancient trees","A fey-touched woodland glade","A place of natural magic","A druidic sanctuary"],
    monolith: ["A towering standing stone","An obelisk of unknown origin","A crystalline pillar","A carved stone monument","An arcane beacon stone"],
    mine: ["An abandoned mining operation","A deep ore mine","A played-out quarry","A collapsed shaft with secrets below","A dwarven mining outpost"],
    fortress: ["A ruined hilltop fortification","A partially intact military outpost","A strategic defensive position","A border garrison","A warlord's stronghold"],
    temple: ["A grand religious edifice","A cathedral to the old gods","A holy sanctuary","A cursed place of worship","A monastery in the wilds"],
    lighthouse: ["A coastal beacon tower","A storm-battered signal point","A harbor watchtower","A cliff-side lighthouse","A navigational landmark"],
    camp: ["A bandit encampment","A nomadic tribe's camp","A military bivouac","A ranger outpost","A merchant caravan rest"],
  };
  const arr = descs[type] || descs.ruins;
  return arr[Math.floor(rng() * arr.length)];
}

// ─── LANDMARK/GEOGRAPHIC FEATURE GENERATION ────────────────────────────────

function generateLandmarks(heightmap, moistureMap, gridW, gridH, rng) {
  const landmarks = [];
  const seaLevel = 0.38;

  // Find mountain peak clusters
  for (let i = 0; i < 6; i++) {
    let bestX = -1, bestY = -1, bestH = 0;
    for (let attempt = 0; attempt < 200; attempt++) {
      ...
      if (h > bestH && h > 0.8) {
        ...
        for (const lm of landmarks) {
          if (Math.sqrt((lm.x - x) ** 2 + (lm.y - y) ** 2) < 30) tooClose = true;
        }
        if (!tooClose) { bestH = h; bestX = x; bestY = y; }
      }
    }
    if (bestX >= 0) {
      landmarks.push({ x: bestX, y: bestY, type: "mountain_range", name: generateName(rng, "mountain"), h: bestH });
    }
  }

  // Find large forest regions
  for (let i = 0; i < 5; i++) {
    let bestX = -1, bestY = -1, bestM = 0;
    for (let attempt = 0; attempt < 200; attempt++) {
      const x = Math.floor(rng() * gridW);
      const y = Math.floor(rng() * gridH);
      const h = heightmap[y]?.[x] || 0;
      const m = moistureMap[y]?.[x] || 0;
      if (h > seaLevel && h < 0.65 && m > bestM && m > 0.6) {
        let tooClose = false;
        for (const lm of landmarks) {
          if (lm.type === "forest" && Math.sqrt((lm.x - x) ** 2 + (lm.y - y) ** 2) < 25) tooClose = true;
        }
        if (!tooClose) { bestM = m; bestX = x; bestY = y; }
      }
    }
    if (bestX >= 0) {
      landmarks.push({ x: bestX, y: bestY, type: "forest", name: generateName(rng, "forest") });
    }
  }
  ...
  for (let i = 0; i < 3; i++) {
    ...
    for (let attempt = 0; attempt < 200; attempt++) {
      ...
      const h = heightmap[y]?.[x] || 0;
      const m = moistureMap[y]?.[x] || 0;
      if (h > seaLevel && h < 0.6 && m < 0.15) {
        let tooClose = false;
        for (const lm of landmarks) {
          if (lm.type === "desert" && Math.sqrt((lm.x - x) ** 2 + (lm.y - y) ** 2) < 30) tooClose = true;
        }
        if (!tooClose) { bestX = x; bestY = y; }
      }
    }
    if (bestX >= 0) {
      landmarks.push({ x: bestX, y: bestY, type: "desert", name: generateName(rng, "desert") });
    }
  }

  // Find lakes (low points surrounded by land)
  for (let i = 0; i < 5; i++) {
    let bestX = -1, bestY = -1, bestScore = 0;
    for (let attempt = 0; attempt < 300; attempt++) {
      const x = Math.floor(rng() * (gridW - 4)) + 2;
      const y = Math.floor(rng() * (gridH - 4)) + 2;
      const h = heightmap[y][x];
      if (h < seaLevel || h > 0.55) continue;
      // Score by being a local low point
      let isLow = true;
      for (let dy = -2; dy <= 2; dy++) {
        for (let dx = -2; dx <= 2; dx++) {
          if (dx === 0 && dy === 0) continue;
          if ((heightmap[y + dy]?.[x + dx] || 0) < h - 0.02) isLow = false;
        }
      }
      if (isLow) {
        let tooClose = false;
        for (const lm of landmarks) {
          if (lm.type === "lake" && Math.sqrt((lm.x - x) ** 2 + (lm.y - y) ** 2) < 20) tooClose = true;
        }
        if (!tooClose) { bestX = x; bestY = y; break; }
      }
    }
    if (bestX >= 0) {
      landmarks.push({ x: bestX, y: bestY, type: "lake", name: generateName(rng, "lake"), radius: 3 + rng() * 5 });
    }
  }
  ...
  for (let i = 0; i < 3; i++) {
    ...
    for (let attempt = 0; attempt < 200; attempt++) {
      ...
      const h = heightmap[y]?.[x] || 0;
      const m = moistureMap[y]?.[x] || 0;
      if (h > 0.38 && h < 0.45 && m > 0.6) {
        bestX = x; bestY = y; break;
      }
    }
    if (bestX >= 0) {
      landmarks.push({ x: bestX, y: bestY, type: "swamp", name: generateName(rng, "swamp") });
    }
  }
...
// ─── SVG MAP RENDERER HELPERS ──────────────────────────────────────────────

function biomeToSVGColor(biome) {
  return BIOMES[biome]?.color || "#888";
}

// Smooth SVG path from grid points
function pointsToSmoothPath(points, cellSize, closed = false) {
  if (points.length < 2) return "";
  const pts = points.map(p => ({ x: p.x * cellSize, y: p.y * cellSize }));
...
function hullToSmoothPath(hull, cellSize) {
  if (hull.length < 3) return "";
  const pts = hull.map(p => ({ x: p.x * cellSize, y: p.y * cellSize }));
  // Catmull-Rom to Bezier conversion for smooth territory borders
  let d = "";
...
function generateWorld(seed, config) {
  ...
  const gridW = config.gridWidth || 160;
  const gridH = config.gridHeight || 120;

  // Generate terrain layers
  const heightmap = generateHeightmap(gridW, gridH, mulberry32(hashStr(seed + "_h")),
    config.heightOctaves || 6, config.heightPersistence || 0.52, config.heightLacunarity || 2.1);
  const moisture = generateMoisture(gridW, gridH, mulberry32(hashStr(seed + "_m")));
  const temperature = generateTemperature(gridW, gridH, mulberry32(hashStr(seed + "_t")), config.latitudeBias !== false);
  ...
  if (config.continentShape === "pangaea") {
    const cx = gridW * 0.45, cy = gridH * 0.5;
    for (let y = 0; y < gridH; y++) {
      for (let x = 0; x < gridW; x++) {
        const dx = (x - cx) / (gridW * 0.4);
        const dy = (y - cy) / (gridH * 0.4);
        const dist = Math.sqrt(dx * dx + dy * dy);
        heightmap[y][x] += Math.max(0, 0.3 * (1 - dist));
      }
    }
  } else if (config.continentShape === "archipelago") {
    // Create multiple island centers
    const centers = [];
    const islandRng = mulberry32(hashStr(seed + "_islands"));
    for (let i = 0; i < 8; i++) {
      centers.push({
        x: gridW * 0.15 + islandRng() * gridW * 0.7,
        y: gridH * 0.15 + islandRng() * gridH * 0.7,
        r: 10 + islandRng() * 20,
      });
    }
    for (let y = 0; y < gridH; y++) {
  ...
  // Adjust sea level based on config
  const seaLevelAdj = config.seaLevel || 0;
  if (seaLevelAdj !== 0) {
    for (let y = 0; y < gridH; y++) {
      for (let x = 0; x < gridW; x++) {
        heightmap[y][x] = Math.max(0, Math.min(1, heightmap[y][x] - seaLevelAdj * 0.1));
      }
    }
  }
  ...
  for (let y = 0; y < gridH; y++) {
    biomeGrid[y] = [];
    for (let x = 0; x < gridW; x++) {
      biomeGrid[y][x] = classifyBiome(heightmap[y][x], moisture[y][x], temperature[y][x]);
    }
  }

  // Generate features
  const rivers = generateRivers(heightmap, moisture, gridW, gridH, mulberry32(hashStr(seed + "_r")), config.riverCount || 10);
  const settlements = placeCities(heightmap, moisture, rivers, gridW, gridH, mulberry32(hashStr(seed + "_c")), config);
  const roads = generateRoads(settlements, heightmap, gridW, gridH, mulberry32(hashStr(seed + "_rd")));
  const kingdoms = generateKingdoms(settlements, heightmap, gridW, gridH, mulberry32(hashStr(seed + "_k")), config.kingdomCount || 4);
  const pois = generatePOIs(heightmap, moisture, settlements, gridW, gridH, mulberry32(hashStr(seed + "_p")), config);
  const landmarks = generateLandmarks(heightmap, moisture, gridW, gridH, mulberry32(hashStr(seed + "_l")));

  // Generate sea names for large water bodies
  const seas = [];
  const seaRng = mulberry32(hashStr(seed + "_sea"));
  // Find water clusters using flood fill on biomeGrid
  const waterVisited = Array.from({ length: gridH }, () => new Uint8Array(gridW));
  for (let y = 0; y < gridH; y++) {
    for (let x = 0; x < gridW; x++) {
      ...
      if ((b === "deepOcean" || b === "ocean") && !waterVisited[y][x]) {
        ...
        while (queue.length > 0) {
          ...
          for (const [dx, dy] of [[1,0],[-1,0],[0,1],[0,-1]]) {
            const nx = cx + dx, ny = cy + dy;
            if (nx >= 0 && nx < gridW && ny >= 0 && ny < gridH && !waterVisited[ny][nx]) {
              const nb = biomeGrid[ny][nx];
              if (nb === "deepOcean" || nb === "ocean") {
        ...
        if (cells.length > 50) {
          ...
          seas.push({
            x: avgX, y: avgY, size: cells.length,
            name: NAME_STANDALONE.sea[Math.floor(seaRng() * NAME_STANDALONE.sea.length)],
          });
        }
      }
    }
  ...
  return {
    seed, config, gridW, gridH,
    heightmap, moisture, temperature, biomeGrid,
    rivers, settlements, roads, kingdoms, pois, landmarks, seas,
  };
}

// ═══════════════════════════════════════════════════════════════════════════
// FANTASY MAP GENERATOR — REACT COMPONENT
// ═══════════════════════════════════════════════════════════════════════════
...
function FantasyMapGenerator() {
  ...
  const [config, setConfig] = useState({
    ...
    seaLevel: 0,
    heightOctaves: 6,
    heightPersistence: 0.52,
    heightLacunarity: 2.1,
    latitudeBias: true,
    riverCount: 10,
    capitals: 3,
    cities: 8,
  ...
  const mapW = (config.gridWidth || 160) * CELL;
  const mapH = (config.gridHeight || 120) * CELL;

  // ─── GENERATION ────────────────────────────────────────────────────────

  const doGenerate = useCallback((s, c) => {
    setGenerating(true);
    // Use setTimeout to allow React to render "generating" state
    setTimeout(() => {
      try {
        const w = generateWorld(s || seed, c || config);
        setWorld(w);
        setHistory(prev => [...prev.slice(-19), { seed: s || seed, config: c || config }]);
  ...
  // Generate on mount
  useEffect(() => {
    doGenerate(seed, config);
  }, []);

  const randomSeed = () => {
    const words = ["Dragon","Storm","Shadow","Crystal","Iron","Thunder","Moon","Star","Frost","Fire","Blood","Ghost","Elder","Arcane","Mystic","Void","Dawn","Dusk","Wild","Bone"];
  ...
  const centerOn = useCallback((x, y) => {
    setPan({ x: -x * CELL + mapW / 2 / zoom, y: -y * CELL + mapH / 2 / zoom });
  ...
  const searchResults = useCallback(() => {
    ...
    for (const s of world.settlements) {
      if (s.name.toLowerCase().includes(q)) results.push({ ...s, category: "settlement" });
    }
    for (const p of world.pois) {
      if (p.name.toLowerCase().includes(q)) results.push({ ...p, category: "poi" });
    }
    for (const k of world.kingdoms) {
      if (k.name.toLowerCase().includes(q)) results.push({ ...k, category: "kingdom" });
    }
    for (const r of world.rivers) {
      if (r.name.toLowerCase().includes(q)) results.push({ ...r, category: "river" });
    }
    for (const l of world.landmarks) {
      if (l.name.toLowerCase().includes(q)) results.push({ ...l, category: "landmark" });
    }
  ...
  const exportSVG = useCallback(() => {
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `${seed}-fantasy-map.svg`;
    a.click();
  ...
  const exportJSON = useCallback(() => {
    ...
    const exportData = {
      ...
      rivers: world.rivers.map(r => ({ name: r.name, length: r.path.length })),
      pois: world.pois,
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `${seed}-world-data.json`;
    a.click();
    URL.revokeObjectURL(url);
  }, [world, seed]);
  ...
  const showLabelsAtZoom = zoom > 0.6;
  const showSettlementDetails = zoom > 1.2;
  const showPOILabels = zoom > 1.8;
  const showSmallSettlements = zoom > 0.8;
  const showHamlets = zoom > 2;

  // Terrain cell rendering optimization: downsample at low zoom
  const renderStep = zoom < 0.4 ? 3 : zoom < 0.8 ? 2 : 1;

  return (
    <div style={{ display:"flex", height:"calc(100vh - 56px)", overflow:"hidden", background:T.bg }}>

      {/* ═══ LEFT SIDEBAR ═══ */}
      <div style={{ width:320, flexShrink:0, borderRight:`1px solid ${T.border}`, display:"flex", flexDirection:"column", background:T.bgNav, overflow:"hidden" }}>

        {/* Seed Controls */}
        <div style={{ padding:16, borderBottom:`1px solid ${T.border}` }}>
          <div style={{ fontFamily:T.ui, fontSize:9, letterSpacing:"2px", color:T.crimson, textTransform:"uppercase", fontWeight:500, marginBottom:10, display:"flex", alignItems:"center", gap:6 }}>
            <Wand2 size={12}/> Map Forge
          </div>
          <div style={{ display:"flex", gap:6, marginBottom:8 }}>
            <Input value={seedInput} onChange={setSeedInput} placeholder="World seed..." style={{ flex:1, fontSize:12, padding:"6px 10px" }} />
            <button onClick={randomSeed} title="Random seed" style={{ background:T.bgCard, border:`1px solid ${T.border}`, borderRadius:2, cursor:"pointer", padding:"6px 10px", color:T.textMuted, display:"flex", alignItems:"center" }}>
              <RefreshCw size={12}/>
            </button>
          </div>
          <div style={{ display:"flex", gap:6 }}>
            ...
            <CrimsonBtn small secondary onClick={() => setShowConfig(!showConfig)}>
              <Settings size={10}/>
            </CrimsonBtn>
          </div>
        </div>
        ...
        {showConfig && (
          <div style={{ padding:16, borderBottom:`1px solid ${T.border}`, maxHeight:400, overflowY:"auto" }}>
            <div style={{ fontFamily:T.ui, fontSize:8, letterSpacing:"1px", color:T.textMuted, textTransform:"uppercase", marginBottom:10 }}>World Configuration</div>

            <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
              <div>
                <label style={{ fontSize:10, color:T.textDim, fontFamily:T.body, display:"block", marginBottom:4 }}>Continent Shape</label>
                <Select value={config.continentShape} onChange={v => setConfig(c => ({...c, continentShape: v}))} style={{ width:"100%", fontSize:11, padding:"5px 8px" }}>
              ...
              <div style={{ display:"flex", gap:8 }}>
                <div style={{ flex:1 }}>
                  <label style={{ fontSize:10, color:T.textDim, display:"block", marginBottom:4 }}>Width</label>
                  <Input type="number" value={config.gridWidth} onChange={v => setConfig(c => ({...c, gridWidth: parseInt(v)||120}))} style={{ fontSize:11, padding:"5px 8px" }} />
                </div>
                <div style={{ flex:1 }}>
                  <label style={{ fontSize:10, color:T.textDim, display:"block", marginBottom:4 }}>Height</label>
                  <Input type="number" value={config.gridHeight} onChange={v => setConfig(c => ({...c, gridHeight: parseInt(v)||90}))} style={{ fontSize:11, padding:"5px 8px" }} />
                </div>
              </div>

              <div>
                <label style={{ fontSize:10, color:T.textDim, display:"block", marginBottom:4 }}>Sea Level: {config.seaLevel}</label>
                <input type="range" min={-3} max={3} step={1} value={config.seaLevel} onChange={e => setConfig(c => ({...c, seaLevel: parseInt(e.target.value)}))} style={{ width:"100%" }} />
              </div>

              <div>
                <label style={{ fontSize:10, color:T.textDim, display:"block", marginBottom:4 }}>Terrain Detail (Octaves): {config.heightOctaves}</label>
                <input type="range" min={2} max={8} step={1} value={config.heightOctaves} onChange={e => setConfig(c => ({...c, heightOctaves: parseInt(e.target.value)}))} style={{ width:"100%" }} />
              </div>

              <div>
                <label style={{ fontSize:10, color:T.textDim, display:"block", marginBottom:4 }}>Roughness: {config.heightPersistence.toFixed(2)}</label>
                <input type="range" min={0.3} max={0.7} step={0.02} value={config.heightPersistence} onChange={e => setConfig(c => ({...c, heightPersistence: parseFloat(e.target.value)}))} style={{ width:"100%" }} />
              </div>
              ...
              <div style={{ fontFamily:T.ui, fontSize:8, letterSpacing:"1px", color:T.textMuted, textTransform:"uppercase", marginTop:8 }}>Settlements</div>
              {[["capitals","Capitals",1,6],["cities","Cities",2,20],["towns","Towns",5,40],["villages","Villages",0,60],["hamlets","Hamlets",0,80]].map(([key,label,min,max]) => (
                <div key={key}>
                  <label style={{ fontSize:10, color:T.textDim, display:"block", marginBottom:4 }}>{label}: {config[key]}</label>
                  <input type="range" min={min} max={max} step={1} value={config[key]} onChange={e => setConfig(c => ({...c, [key]: parseInt(e.target.value)}))} style={{ width:"100%" }} />
                </div>
              ))}

              <div style={{ fontFamily:T.ui, fontSize:8, letterSpacing:"1px", color:T.textMuted, textTransform:"uppercase", marginTop:8 }}>Features</div>
              {[["riverCount","Rivers",0,20],["kingdomCount","Kingdoms",1,8],["dungeons","Dungeons",0,25],["ruins","Ruins",0,25],["towers","Towers",0,20],["caves","Caves",0,20],["shrines","Shrines",0,20],["mines","Mines",0,15],["fortresses","Fortresses",0,15],["temples","Temples",0,12]].map(([key,label,min,max]) => (
                <div key={key}>
                  <label style={{ fontSize:10, color:T.textDim, display:"block", marginBottom:4 }}>{label}: {config[key]}</label>
                  <input type="range" min={min} max={max} step={1} value={config[key]} onChange={e => setConfig(c => ({...c, [key]: parseInt(e.target.value)}))} style={{ width:"100%" }} />
                </div>
        ...
        <div style={{ display:"flex", borderBottom:`1px solid ${T.border}` }}>
          {[
            ...
            <button key={t.id} onClick={() => setSideTab(t.id)} style={{
              ...
              borderBottom: sideTab === t.id ? `2px solid ${T.crimson}` : "2px solid transparent",
              color: sideTab === t.id ? T.crimson : T.textMuted, display:"flex", alignItems:"center",
              justifyContent:"center", gap:4, fontFamily:T.ui, fontSize:7, letterSpacing:"1px", textTransform:"uppercase",
        ...
        <div style={{ flex:1, overflowY:"auto", padding:16 }}>
          ...
          {sideTab === "overview" && world && (
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              <div style={{ fontSize:16, fontWeight:300, color:T.text, fontFamily:T.body }}>World: {world.seed}</div>
              <div style={{ fontSize:11, color:T.textDim, fontWeight:300 }}>{world.gridW} x {world.gridH} grid ({world.gridW * CELL} x {world.gridH * CELL} px)</div>

              {/* Stats Grid */}
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                {[
                  { label:"Kingdoms", val: world.kingdoms.length, icon: Crown },
                  { label:"Settlements", val: world.settlements.length, icon: MapPin },
                  { label:"Rivers", val: world.rivers.length, icon: Activity },
                  { label:"POIs", val: world.pois.length, icon: Target },
                  { label:"Landmarks", val: world.landmarks.length, icon: Mountain },
                  { label:"Seas", val: world.seas.length, icon: Globe },
                ].map(s => (
                  <div key={s.label} style={{ padding:10, background:T.bgCard, border:`1px solid ${T.borderMid}`, borderRadius:2 }}>
                    <div style={{ display:"flex", alignItems:"center", gap:4, marginBottom:4 }}>
                      <s.icon size={10} color={T.crimson}/>
                      <span style={{ fontFamily:T.ui, fontSize:7, letterSpacing:"1px", color:T.textMuted, textTransform:"uppercase" }}>{s.label}</span>
                    </div>
                    <span style={{ fontSize:20, fontWeight:300, color:T.text }}>{s.val}</span>
                  </div>
                ))}
              </div>
              ...
              {/* Kingdoms List */}
              <div>
                <div style={{ fontFamily:T.ui, fontSize:8, letterSpacing:"1px", color:T.crimson, textTransform:"uppercase", marginBottom:8 }}>Kingdoms</div>
                {world.kingdoms.map((k, i) => (
                  <div key={i} onClick={() => { setSelectedEntity({ ...k, entityType:"kingdom" }); setSideTab("details"); }}
                    style={{ padding:"8px 10px", cursor:"pointer", borderBottom:`1px solid ${T.borderMid}`, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                    <div>
                      <div style={{ fontSize:12, color:T.text, fontWeight:300 }}>{k.name}</div>
                      <div style={{ fontSize:10, color:T.textFaint }}>Capital: {k.capital}</div>
                    </div>
                    <Tag>{k.settlements.length} cities</Tag>
                  </div>
                ))}
              </div>

              {/* Settlement Summary by Type */}
              <div>
                <div style={{ fontFamily:T.ui, fontSize:8, letterSpacing:"1px", color:T.crimson, textTransform:"uppercase", marginBottom:8 }}>Settlements</div>
                {["capital","city","town","village","hamlet"].map(type => {
                  const count = world.settlements.filter(s => s.type === type).length;
                  return (
                    <div key={type} style={{ display:"flex", justifyContent:"space-between", padding:"4px 0", fontSize:11, color:T.textDim }}>
              ...
              {/* Biome Distribution */}
              <div>
                <div style={{ fontFamily:T.ui, fontSize:8, letterSpacing:"1px", color:T.crimson, textTransform:"uppercase", marginBottom:8 }}>Biome Distribution</div>
                {(() => {
                  ...
                  return Object.entries(counts)
                    ...
                    .slice(0, 10)
                    .map(([biome, count]) => (
                      <div key={biome} style={{ marginBottom:4 }}>
                        <div style={{ display:"flex", justifyContent:"space-between", fontSize:10, color:T.textDim, marginBottom:2 }}>
                          <span style={{ display:"flex", alignItems:"center", gap:4 }}>
                            <span style={{ width:8, height:8, background:BIOMES[biome]?.color || "#888", borderRadius:1, display:"inline-block" }}/>
                            {BIOMES[biome]?.label || biome}
                          </span>
                          <span>{(count / total * 100).toFixed(1)}%</span>
                        </div>
                        <div style={{ height:3, background:T.bgInput, borderRadius:1 }}>
                          <div style={{ width:`${count / total * 100}%`, height:"100%", background:BIOMES[biome]?.color || T.crimson, borderRadius:1 }}/>
                        </div>
                      </div>
                    ));
                })()}
              ...
              <div style={{ display:"flex", gap:8, marginTop:8 }}>
                <CrimsonBtn small onClick={exportSVG} style={{ flex:1, justifyContent:"center" }}><Download size={10}/> SVG</CrimsonBtn>
                <CrimsonBtn small secondary onClick={exportJSON} style={{ flex:1, justifyContent:"center" }}><FileText size={10}/> JSON</CrimsonBtn>
              </div>
            </div>
          )}
          ...
          {sideTab === "search" && (
            <div>
              <Input value={searchQuery} onChange={setSearchQuery} placeholder="Search settlements, POIs, kingdoms..." style={{ fontSize:12, marginBottom:12 }} />
              {searchResults().map((r, i) => (
                <div key={i} onClick={() => { setSelectedEntity({ ...r, entityType: r.category }); setSideTab("details"); if (r.x != null) centerOn(r.x, r.y); }}
                  style={{ padding:"8px 10px", cursor:"pointer", borderBottom:`1px solid ${T.borderMid}`, transition:"background 0.15s" }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                    <span style={{ fontSize:12, color:T.text, fontWeight:300 }}>{r.name}</span>
                    <Tag variant="muted">{r.category}</Tag>
                  </div>
                  {r.type && <div style={{ fontSize:10, color:T.textFaint, marginTop:2, textTransform:"capitalize" }}>{r.type}</div>}
                </div>
              ))}
              {searchQuery && searchResults().length === 0 && (
                <p style={{ fontSize:12, color:T.textFaint, textAlign:"center", padding:20, fontStyle:"italic" }}>No results found</p>
              )}
            </div>
          )}
          ...
          {sideTab === "layers" && (
            <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
              <div style={{ fontFamily:T.ui, fontSize:8, letterSpacing:"1px", color:T.textMuted, textTransform:"uppercase", marginBottom:8 }}>Map Layers</div>
              {[
                { key:"terrain", label:"Terrain Colors" },
                ...
                { key:"labels", label:"Labels & Names" },
                { key:"grid", label:"Grid Overlay" },
              ].map(l => (
                <ToggleSwitch key={l.key} on={layers[l.key]} onToggle={() => setLayers(prev => ({...prev, [l.key]:!prev[l.key]}))} label={l.label} />
              ))}

              <div style={{ fontFamily:T.ui, fontSize:8, letterSpacing:"1px", color:T.textMuted, textTransform:"uppercase", marginTop:16, marginBottom:8 }}>Debug Overlays</div>
              {[
              ...
              {selectedEntity ? (
                <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"start" }}>
                    <div>
                      <div style={{ fontSize:16, fontWeight:300, color:T.text }}>{selectedEntity.name}</div>
                      <Tag variant={
                        selectedEntity.entityType === "kingdom" ? "info" :
                        selectedEntity.entityType === "settlement" ? "success" :
                        selectedEntity.entityType === "poi" ? "warning" : "muted"
                      }>{selectedEntity.entityType}</Tag>
                    </div>
                    <button onClick={() => setSelectedEntity(null)} style={{ background:"none", border:"none", cursor:"pointer", color:T.textFaint }}><X size={14}/></button>
                  </div>

                  {selectedEntity.type && (
                    <div style={{ fontSize:12, color:T.textDim }}>
                      <span style={{ fontFamily:T.ui, fontSize:8, letterSpacing:"1px", color:T.textMuted, textTransform:"uppercase", display:"block", marginBottom:4 }}>Type</span>
                      <span style={{ textTransform:"capitalize" }}>{selectedEntity.type}</span>
                    </div>
                  )}

                  {selectedEntity.population != null && (
                    <div style={{ fontSize:12, color:T.textDim }}>
                      <span style={{ fontFamily:T.ui, fontSize:8, letterSpacing:"1px", color:T.textMuted, textTransform:"uppercase", display:"block", marginBottom:4 }}>Population</span>
                      {selectedEntity.population.toLocaleString()}
                    </div>
                  )}

                  {selectedEntity.isCoastal != null && (
                    <div style={{ display:"flex", gap:8 }}>
                      {selectedEntity.isCoastal && <Tag variant="info">Coastal</Tag>}
                      {selectedEntity.isRiverside && <Tag variant="info">Riverside</Tag>}
                    </div>
                  )}

                  {selectedEntity.description && (
                    <div style={{ fontSize:12, color:T.textDim, fontStyle:"italic" }}>
                      <span style={{ fontFamily:T.ui, fontSize:8, letterSpacing:"1px", color:T.textMuted, textTransform:"uppercase", display:"block", marginBottom:4 }}>Description</span>
                      {selectedEntity.description}
                    </div>
                  )}
                  ...
                  {selectedEntity.threat && (
                    <div style={{ fontSize:12, color:T.textDim }}>
                      <span style={{ fontFamily:T.ui, fontSize:8, letterSpacing:"1px", color:T.textMuted, textTransform:"uppercase", display:"block", marginBottom:4 }}>Threat Level</span>
                      <Tag variant={selectedEntity.threat === "extreme" ? "critical" : selectedEntity.threat === "high" ? "danger" : selectedEntity.threat === "medium" ? "warning" : "success"}>{selectedEntity.threat}</Tag>
                    </div>
                  )}

                  {selectedEntity.capital && (
                    <div style={{ fontSize:12, color:T.textDim }}>
                      <span style={{ fontFamily:T.ui, fontSize:8, letterSpacing:"1px", color:T.textMuted, textTransform:"uppercase", display:"block", marginBottom:4 }}>Capital</span>
                      {selectedEntity.capital}
                    </div>
                  )}

                  {selectedEntity.settlements && (
                    <div>
                      <span style={{ fontFamily:T.ui, fontSize:8, letterSpacing:"1px", color:T.textMuted, textTransform:"uppercase", display:"block", marginBottom:4 }}>Settlements ({selectedEntity.settlements.length})</span>
                      {selectedEntity.settlements.map((s, i) => (
                        <div key={i} style={{ fontSize:11, color:T.textDim, padding:"2px 0" }}>{s}</div>
                      ))}
                    </div>
                  )}

                  {selectedEntity.x != null && (
                    <CrimsonBtn small onClick={() => centerOn(selectedEntity.x, selectedEntity.y)} style={{ marginTop:8 }}>
                      <Compass size={10}/> Center on Map
                    </CrimsonBtn>
                  )}
                </div>
              ) : (
                <div style={{ textAlign:"center", padding:32, color:T.textFaint }}>
                  ...
                  <p style={{ fontSize:12, fontWeight:300 }}>Click any settlement, POI, or kingdom on the map to view details</p>
                </div>
              )}
            </div>
          )}
      ...
      <div style={{ flex:1, position:"relative", overflow:"hidden", cursor: isPanning ? "grabbing" : "grab" }}
        ...
        {/* Loading Overlay */}
        {generating && (
          <div style={{ position:"absolute", inset:0, zIndex:100, background:"rgba(0,0,0,0.7)", display:"flex", alignItems:"center", justifyContent:"center", flexDirection:"column", gap:16 }}>
            <RefreshCw size={32} color={T.crimson} style={{ animation:"spin 1s linear infinite" }}/>
            <span style={{ fontFamily:T.ui, fontSize:11, letterSpacing:"2px", color:T.crimson, textTransform:"uppercase" }}>Forging World...</span>
          </div>
        )}
        ...
        {world && (
          <svg ref={svgRef}
            ...
            viewBox={`${-pan.x * zoom} ${-pan.y * zoom} ${(config.gridWidth * CELL) / zoom * 1.5} ${(config.gridHeight * CELL) / zoom * 1.5}`}
            style={{ background:BIOMES.deepOcean.color }}>

            <defs>
              {/* Parchment texture */}
              <filter id="parchment">
                <feTurbulence type="fractalNoise" baseFrequency="0.04" numOctaves="4" result="noise"/>
                <feDiffuseLighting in="noise" lightingColor="#f5e6c8" surfaceScale="1" result="light">
              ...
              {/* Water pattern */}
              <pattern id="waterPattern" width="12" height="12" patternUnits="userSpaceOnUse">
                <rect width="12" height="12" fill={BIOMES.ocean.color}/>
                <path d="M0 6 Q3 4 6 6 Q9 8 12 6" stroke="rgba(255,255,255,0.06)" strokeWidth="0.5" fill="none"/>
              </pattern>

              {/* Mountain hatch */}
              <pattern id="mountainHatch" width="6" height="6" patternUnits="userSpaceOnUse" patternTransform="rotate(45)">
                <line x1="0" y1="0" x2="0" y2="6" stroke="rgba(0,0,0,0.1)" strokeWidth="0.5"/>
              </pattern>

              {/* Forest dot pattern */}
              <pattern id="forestDots" width="8" height="8" patternUnits="userSpaceOnUse">
                <circle cx="2" cy="2" r="1" fill="rgba(0,80,0,0.15)"/>
                <circle cx="6" cy="6" r="1.2" fill="rgba(0,80,0,0.1)"/>
              </pattern>
            ...
            {layers.terrain && (
              <g>
                {(() => {
                  const rects = [];
                  for (let y = 0; y < world.gridH; y += renderStep) {
                    for (let x = 0; x < world.gridW; x += renderStep) {
                      const biome = world.biomeGrid[y][x];
                      const color = biomeToSVGColor(biome);
                      rects.push(
                        <rect key={`${x},${y}`} x={x * CELL} y={y * CELL} width={CELL * renderStep + 0.5} height={CELL * renderStep + 0.5} fill={color}/>
                      );
                    }
                  }
                  return rects;
            ...
            {layers.heightmap && (
              <g opacity={0.6}>
                {(() => {
                  ...
                  for (let y = 0; y < world.gridH; y += renderStep) {
                    for (let x = 0; x < world.gridW; x += renderStep) {
                      const h = world.heightmap[y][x];
                      const v = Math.floor(h * 255);
                      rects.push(
                        <rect key={`h${x},${y}`} x={x * CELL} y={y * CELL} width={CELL * renderStep + 0.5} height={CELL * renderStep + 0.5} fill={`rgb(${v},${v},${v})`}/>
                      );
                    }
                  }
                  return rects;
            ...
            {layers.moisture && (
              <g opacity={0.6}>
                {(() => {
                  ...
                  for (let y = 0; y < world.gridH; y += renderStep) {
                    for (let x = 0; x < world.gridW; x += renderStep) {
                      const m = world.moisture[y][x];
                      rects.push(
                        <rect key={`m${x},${y}`} x={x * CELL} y={y * CELL} width={CELL * renderStep + 0.5} height={CELL * renderStep + 0.5} fill={`rgb(0,${Math.floor(m*200)},${Math.floor(m*255)})`}/>
                      );
                    }
                  }
                  return rects;
            ...
            {layers.temperature && (
              <g opacity={0.6}>
                {(() => {
                  ...
                  for (let y = 0; y < world.gridH; y += renderStep) {
                    for (let x = 0; x < world.gridW; x += renderStep) {
                      const t = world.temperature[y][x];
                      rects.push(
                        <rect key={`t${x},${y}`} x={x * CELL} y={y * CELL} width={CELL * renderStep + 0.5} height={CELL * renderStep + 0.5} fill={`rgb(${Math.floor(t*255)},${Math.floor((1-Math.abs(t-0.5)*2)*100)},${Math.floor((1-t)*255)})`}/>
                      );
                    }
                  }
                  return rects;
            ...
            {/* ── GRID OVERLAY ── */}
            {layers.grid && (
              <g opacity={0.15} stroke={T.text} strokeWidth={0.3}>
                {Array.from({ length: Math.ceil(world.gridW / 10) + 1 }, (_, i) => (
                  <line key={`gv${i}`} x1={i * 10 * CELL} y1={0} x2={i * 10 * CELL} y2={world.gridH * CELL}/>
                ))}
                {Array.from({ length: Math.ceil(world.gridH / 10) + 1 }, (_, i) => (
                  <line key={`gh${i}`} x1={0} y1={i * 10 * CELL} x2={world.gridW * CELL} y2={i * 10 * CELL}/>
                ))}
              </g>
            )}
            ...
            {layers.kingdoms && world.kingdoms.map((k, i) => (
              <g key={`k${i}`}>
                <path d={hullToSmoothPath(k.hull, CELL)} fill={k.color} stroke={k.borderColor} strokeWidth={2} strokeDasharray="8,4"/>
                {showLabelsAtZoom && layers.labels && (
                  <text x={k.hull.reduce((s,p) => s + p.x, 0) / k.hull.length * CELL}
                    y={k.hull.reduce((s,p) => s + p.y, 0) / k.hull.length * CELL}
                    textAnchor="middle" fill={k.borderColor.replace("0.5","0.8")}
                    fontFamily="'Cinzel', serif" fontSize={zoom > 1 ? 10 : 14} fontWeight={500}
                    letterSpacing="3px" style={{ textTransform:"uppercase" }} filter="url(#labelShadow)">
                    {k.name}
                  </text>
                )}
              </g>
            ...
            {layers.rivers && world.rivers.map((r, i) => (
              <g key={`r${i}`}>
                <path d={pointsToSmoothPath(r.path, CELL)} fill="none" stroke="#3a7ab0" strokeWidth={r.width} strokeLinecap="round" strokeLinejoin="round" opacity={0.8}/>
                {showLabelsAtZoom && layers.labels && r.path.length > 10 && (
                  <text x={r.path[Math.floor(r.path.length / 3)].x * CELL}
                    y={r.path[Math.floor(r.path.length / 3)].y * CELL - 4}
                    fill="#5a9ac8" fontSize={8} fontFamily="'Spectral', serif" fontStyle="italic" filter="url(#labelShadow)">
                    {r.name}
                  </text>
                )}
              </g>
            ...
            {/* ── ROADS ── */}
            {layers.roads && world.roads.map((r, i) => (
              <path key={`rd${i}`}
                d={pointsToSmoothPath(r.path, CELL)}
                fill="none"
                stroke={r.type === "major" ? "rgba(180,140,80,0.5)" : "rgba(150,130,90,0.3)"}
                strokeWidth={r.type === "major" ? 2 : 1}
                strokeDasharray={r.type === "major" ? "none" : "4,3"}
                strokeLinecap="round"/>
            ))}

            {/* ── LANDMARKS (labels) ── */}
            {layers.labels && showLabelsAtZoom && world.landmarks.map((l, i) => (
              <text key={`lm${i}`} x={l.x * CELL} y={l.y * CELL}
                textAnchor="middle" fill="rgba(255,255,255,0.5)"
                fontFamily="'Cinzel', serif" fontSize={l.type === "mountain_range" ? 11 : 9}
                fontStyle={l.type === "forest" ? "italic" : "normal"}
                letterSpacing="2px" style={{ textTransform:"uppercase" }} filter="url(#labelShadow)">
                {l.name}
              </text>
            ))}

            {/* ── SEA NAMES ── */}
            {layers.labels && world.seas.map((s, i) => (
              <text key={`sea${i}`} x={s.x * CELL} y={s.y * CELL}
                textAnchor="middle" fill="rgba(100,160,220,0.4)"
                fontFamily="'Cinzel', serif" fontSize={s.size > 500 ? 16 : 11}
                letterSpacing="6px" fontStyle="italic" style={{ textTransform:"uppercase" }} filter="url(#labelShadow)">
                {s.name}
              </text>
            ))}
            ...
            {layers.settlements && world.settlements.map((s, i) => {
              ...
              if ((s.type === "village" || s.type === "hamlet") && !showSmallSettlements) return null;
              const size = s.type === "capital" ? 6 : s.type === "city" ? 4.5 : s.type === "town" ? 3.5 : s.type === "village" ? 2.5 : 2;
              const isSelected = selectedEntity?.name === s.name && selectedEntity?.entityType === "settlement";
              return (
                <g key={`s${i}`}
                  onClick={(e) => { e.stopPropagation(); handleEntityClick({ ...s, entityType:"settlement" }); }}
                  onMouseEnter={() => setHoveredEntity(s)}
                  onMouseLeave={() => setHoveredEntity(null)}
                  style={{ cursor:"pointer" }}>
                  {isSelected && <circle cx={s.x * CELL} cy={s.y * CELL} r={size + 4} fill="none" stroke={T.crimson} strokeWidth={1.5} filter="url(#selectedGlow)"/>}
                  {s.type === "capital" ? (
                    <>
                      <rect x={s.x * CELL - size} y={s.y * CELL - size} width={size * 2} height={size * 2} fill="#ffd700" stroke="#8b6914" strokeWidth={1} transform={`rotate(45,${s.x * CELL},${s.y * CELL})`}/>
                      <circle cx={s.x * CELL} cy={s.y * CELL} r={size * 0.5} fill="#8b6914"/>
                    </>
                  ) : s.type === "city" ? (
                    <circle cx={s.x * CELL} cy={s.y * CELL} r={size} fill="#e8d8a0" stroke="#8b7a3a" strokeWidth={1}/>
                  ) : s.type === "town" ? (
                    <circle cx={s.x * CELL} cy={s.y * CELL} r={size} fill="#c8b878" stroke="#8b7a3a" strokeWidth={0.5}/>
                  ) : (
                    <circle cx={s.x * CELL} cy={s.y * CELL} r={size} fill="#a89868" stroke="#6b5a2a" strokeWidth={0.3}/>
                  )}
                  {showLabelsAtZoom && layers.labels && (s.type === "capital" || s.type === "city" || (showSettlementDetails && s.type === "town")) && (
                    <text x={s.x * CELL} y={s.y * CELL - size - 3}
                      textAnchor="middle" fill="#f0e8d0"
                      fontFamily="'Cinzel', serif" fontSize={s.type === "capital" ? 9 : 7} fontWeight={s.type === "capital" ? 700 : 400}
                      letterSpacing={s.type === "capital" ? "1.5px" : "0.5px"} filter="url(#labelShadow)">
                      {s.name}
                    </text>
                  )}
                </g>
            ...
            {layers.pois && world.pois.map((p, i) => {
              if (!showSmallSettlements && !p.discovered) return null;
              const isSelected = selectedEntity?.name === p.name && selectedEntity?.entityType === "poi";
              const iconColor = p.type === "dungeon" || p.type === "cave" ? "#8a5050" :
                               p.type === "ruins" || p.type === "monolith" ? "#8a8070" :
                               p.type === "tower" || p.type === "fortress" || p.type === "lighthouse" ? "#7a7a8a" :
                               p.type === "shrine" || p.type === "temple" ? "#7a8a7a" :
                               p.type === "grove" ? "#4a7a3a" :
                               p.type === "mine" ? "#8a7050" :
                               p.type === "camp" ? "#8a6a50" : "#888";
              return (
                <g key={`p${i}`}
                  onClick={(e) => { e.stopPropagation(); handleEntityClick({ ...p, entityType:"poi" }); }}
                  onMouseEnter={() => setHoveredEntity(p)}
                  onMouseLeave={() => setHoveredEntity(null)}
                  style={{ cursor:"pointer" }}>
                  {isSelected && <circle cx={p.x * CELL} cy={p.y * CELL} r={8} fill="none" stroke={T.crimson} strokeWidth={1.5} filter="url(#selectedGlow)"/>}
                  <circle cx={p.x * CELL} cy={p.y * CELL} r={3} fill={iconColor} stroke="rgba(0,0,0,0.3)" strokeWidth={0.5}/>
                  {p.type === "dungeon" && <path d={`M${p.x*CELL-2} ${p.y*CELL+1} L${p.x*CELL} ${p.y*CELL-3} L${p.x*CELL+2} ${p.y*CELL+1}`} fill="none" stroke="#c06050" strokeWidth={0.8}/>}
                  {p.type === "tower" && <line x1={p.x*CELL} y1={p.y*CELL-4} x2={p.x*CELL} y2={p.y*CELL+1} stroke="#8888aa" strokeWidth={1}/>}
                  {p.type === "shrine" && <circle cx={p.x*CELL} cy={p.y*CELL} r={4} fill="none" stroke="#7a9a7a" strokeWidth={0.5} strokeDasharray="2,2"/>}
                  {showPOILabels && layers.labels && (
                    <text x={p.x * CELL} y={p.y * CELL - 6}
                      textAnchor="middle" fill="rgba(220,200,180,0.7)"
                      fontFamily="'Spectral', serif" fontSize={6} fontStyle="italic" filter="url(#labelShadow)">
                      {p.name}
                    </text>
                  )}
                </g>
            ...
            {hoveredEntity && !isPanning && (
              <g>
                <rect x={hoveredEntity.x * CELL + 10} y={hoveredEntity.y * CELL - 30}
                  width={Math.max(100, hoveredEntity.name.length * 7 + 20)} height={38}
                  rx={2} fill="rgba(20,20,25,0.9)" stroke="rgba(150,80,80,0.4)" strokeWidth={0.5}/>
                <text x={hoveredEntity.x * CELL + 20} y={hoveredEntity.y * CELL - 15}
                  fill="#f0e0c0" fontFamily="'Cinzel', serif" fontSize={9} fontWeight={500}>
                  {hoveredEntity.name}
                </text>
                <text x={hoveredEntity.x * CELL + 20} y={hoveredEntity.y * CELL - 3}
                  fill="rgba(200,180,150,0.6)" fontFamily="'Spectral', serif" fontSize={7}
                  style={{ textTransform:"capitalize" }}>
                  {hoveredEntity.type}{hoveredEntity.population ? ` — Pop. ${hoveredEntity.population.toLocaleString()}` : ""}
                </text>
              </g>
            )}
          </svg>
        ...
        <div style={{ position:"absolute", bottom:16, left:16, display:"flex", gap:8, zIndex:50 }}>
          {/* Zoom controls */}
          <div style={{ background:"rgba(20,20,25,0.85)", border:`1px solid ${T.border}`, borderRadius:2, padding:"6px 8px", display:"flex", alignItems:"center", gap:8 }}>
            <button onClick={() => setZoom(z => Math.max(0.15, z * 0.7))} style={{ background:"none", border:"none", cursor:"pointer", color:T.textMuted, padding:2 }}>
              <Minus size={12}/>
            </button>
            <span style={{ fontFamily:T.ui, fontSize:8, color:T.textDim, letterSpacing:"1px", minWidth:40, textAlign:"center" }}>{(zoom * 100).toFixed(0)}%</span>
            <button onClick={() => setZoom(z => Math.min(8, z * 1.3))} style={{ background:"none", border:"none", cursor:"pointer", color:T.textMuted, padding:2 }}>
              <Plus size={12}/>
            </button>
          </div>

          {/* Zoom presets */}
          <div style={{ background:"rgba(20,20,25,0.85)", border:`1px solid ${T.border}`, borderRadius:2, padding:"6px 8px", display:"flex", gap:6 }}>
            {zoomPresets.map(p => (
              <button key={p.label} onClick={() => setZoom(p.zoom)} style={{
                background: Math.abs(zoom - p.zoom) < 0.1 ? T.crimsonSoft : "transparent",
                border:"none", cursor:"pointer", padding:"3px 8px", borderRadius:2,
                fontFamily:T.ui, fontSize:7, letterSpacing:"1px", color: Math.abs(zoom - p.zoom) < 0.1 ? T.crimson : T.textMuted,
        ...
        {/* Coordinates */}
        <div style={{ position:"absolute", bottom:16, right:16, background:"rgba(20,20,25,0.85)", border:`1px solid ${T.border}`, borderRadius:2, padding:"6px 10px", zIndex:50 }}>
          <span style={{ fontFamily:T.ui, fontSize:7, letterSpacing:"1px", color:T.textFaint }}>
            {world ? `${world.gridW}x${world.gridH} — Seed: ${world.seed}` : "No world"}
          </span>
        </div>

        {/* Minimap */}
        {world && (
          <div style={{ position:"absolute", top:16, right:16, width:180, height:135, background:"rgba(20,20,25,0.9)", border:`1px solid ${T.border}`, borderRadius:2, overflow:"hidden", zIndex:50 }}>
            <svg width="180" height="135" viewBox={`0 0 ${world.gridW} ${world.gridH}`}>
              ...
              {(() => {
                ...
                for (let y = 0; y < world.gridH; y += step) {
                  for (let x = 0; x < world.gridW; x += step) {
                    rects.push(
                      <rect key={`mm${x},${y}`} x={x} y={y} width={step} height={step} fill={biomeToSVGColor(world.biomeGrid[y][x])}/>
                    );
                  }
                }
                return rects;
              ...
              {/* Kingdom fills */}
              {world.kingdoms.map((k, i) => (
                <path key={`mmk${i}`} d={hullToSmoothPath(k.hull, 1)} fill={k.color} stroke={k.borderColor} strokeWidth={0.5}/>
              ))}
              {/* Settlements dots */}
              {world.settlements.filter(s => s.type === "capital" || s.type === "city").map((s, i) => (
                <circle key={`mms${i}`} cx={s.x} cy={s.y} r={s.type === "capital" ? 1.5 : 1} fill="#ffd700"/>
              ))}
              {/* Viewport indicator */}
              <rect x={(-pan.x) / CELL} y={(-pan.y) / CELL}
                width={world.gridW / zoom * 0.5} height={world.gridH / zoom * 0.5}
                fill="none" stroke={T.crimson} strokeWidth={1} opacity={0.8}/>
            </svg>
          </div>
        )}
        ...
        {!world && !generating && (
          <div style={{ position:"absolute", inset:0, display:"flex", alignItems:"center", justifyContent:"center", flexDirection:"column", gap:16 }}>
            <MapIcon size={48} color={T.textFaint}/>
            <span style={{ fontFamily:T.ui, fontSize:11, letterSpacing:"2px", color:T.textMuted, textTransform:"uppercase" }}>Enter a seed and click Generate</span>
          </div>
        )}
      </div>
